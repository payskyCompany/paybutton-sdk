package io.paysky.creditCardNfcReader;
...
import io.paysky.creditCardNfcReader.parser.EmvParser;
import io.paysky.creditCardNfcReader.utils.Provider;
...
public class CardNfcAsyncTask extends AsyncTask<Void, Void, Object>{
    ...
    private final static String NFC_POS_V7_TAG = "TAG: Tech [android.nfc.tech.IsoDep]";
    ...
    private CardNfcAsyncTask(Builder b) {
        ...
        if (mTag != null) {
            ...
            try {
                if (mTag.toString().equals(NFC_POS_V7_TAG)
                        || mTag.toString().equals(NFC_A_TAG)
                        || mTag.toString().equals(NFC_B_TAG)) {
                    execute();
                } else {
                    if (!b.mFromStart) {
